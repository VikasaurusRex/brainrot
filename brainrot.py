...
import whisper
from ass_highlight import generate_ass_highlight
...
TOPIC = "BRAT diet for recovering from diarrhea"  # Example topic, can be changed
...
FONT_SIZE = 24  # Reasonable size for 1920px height video
...
def create_simple_subtitles(audio_path, script, output_dir):
    ...
    try:
        ...
        ass_highlight_file = output_dir / "subtitles_highlight.ass"
        ...
        print(f"✅ Subtitle files created: {subtitle_file}, {word_subtitle_file}, {ass_highlight_file}")
        return True
    except Exception as e:
...
def create_ass_highlight_subtitles(segments, output_path):
    """Generate ASS subtitle file with per-word highlight (green)"""
...
def create_fallback_subtitles(audio_path, script, output_dir):
    ...
    try:
        ...
        word_srt_path = create_word_srt_from_fallback(word_segments, output_dir / "word_subtitles.srt")
...
def render_video(audio_path, output_dir):
    ...
    ass_highlight_abs_path = (output_dir / "subtitles_highlight.ass").resolve().as_posix()
    ...
    # Build filter_complex string using only the ASS highlight file
    filter_complex = (
...
def main():
    ...
    timestamp = "1749714653" # str(int(time.time()))
